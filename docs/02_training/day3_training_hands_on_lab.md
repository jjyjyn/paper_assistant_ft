...
- 这不应被解释成“模型突然变差”。
- 更合理的结论是：
  - 前两轮的分数被 <think> 长文本污染了
  - 第三轮把推理链和最终答案切开后，才看到了真实有效答案质量
...
2. Raw Prediction
- 这是模型原始输出。
- 如果这里有大量 <think>，而 Prediction 只有残句或空串，就说明此前的分数虚高。
...
- 含义：不是完全不会，而是最终答案通道崩了。

2. 1_0062
- Raw Prediction 仍有方法分析思路。
- Prediction 为空。
- 含义：先前字符重合主要来自推理文本，不是有效答案。

### 从面试角度怎么讲

- “第三轮评测的分数变低，反而说明评测口径更干净了。”
- “我不是把 <think> 留在答案里继续自欺欺人，而是把推理链剥离后重新看模型真正交付给用户的内容。”
- “这一步让我把问题进一步收敛到了输出控制，而不是继续停留在脏指标上。”

### 下一步该干什么

1. 回看训练样本的目标答案格式是否足够稳定。
2. 检查是否需要更明确的模板化指令。
3. 继续做“结构正确率/空答案率”一类更贴近当前问题的补充指标。
