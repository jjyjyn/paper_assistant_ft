...
## 2026-03-12：第三轮清洗后评测完成，确认首轮分数存在 <think> 虚高

### 执行动作

- 在服务器上确认新版 scripts/eval_lora_model.py 已生效，能够搜到：
  - strip_think_content
  - aw_prediction
  - cleaned_prediction
- 重新运行：
  - ash scripts/run_eval_v1.sh
- 产出新目录：
...
- 	est_v1
  - exact_match_rate = 0.0000
  - vg_char_f1 = 0.0050335570469798654
- external_eval_v1
  - exact_match_rate = 0.0000
  - vg_char_f1 = 0.0000
...
- 首轮与第二轮：
  - 	est_v1 avg_char_f1 ≈ 0.3804
  - external_eval_v1 avg_char_f1 ≈ 0.4090
- 第三轮：
  - 	est_v1 avg_char_f1 ≈ 0.0050
  - external_eval_v1 avg_char_f1 = 0.0000
- 报告结构也发生变化：
  - 现在有 ### Prediction
  - 也有 ### Raw Prediction
  - <think> 只出现在 Raw Prediction，说明清洗逻辑终于真正作用到了最终评分通道

### 具体样例解释

1. 1_0012
- Raw Prediction 中仍有完整 <think> ... </think>。
- Prediction 被清洗后只剩：
  - 以下是模拟答辩老师追问的三
- 这说明模型不是完全没有方向，而是最终答案通道只留下了残句。

2. 1_0062
- Raw Prediction 中仍是长段自我分析。
- Prediction 清洗后变成空字符串。
- 这说明此前的部分 char_f1 不是有效答案质量，而是推理文本与参考答案发生了字符级重叠。

3. external_eval_v1
- 多个样例在清洗后直接归零。
- 说明外部集上更明显的问题不是“答错一点”，而是“最终答案几乎没有成功落地”。

### 工程结论

- 第三轮不是“结果更差了所以修坏了”，而是第一次得到更可信的评测口径。
- 前两轮的较高 char_f1 主要是 <think> 泄露造成的脏分数。
- 第三轮暴露出的真实瓶颈是：
  - 输出控制失败
  - 答案截断
  - 模板收束失败
...
- 它证明了我会区分：
  ...
  - 版本同步问题
  - 输出通道问题
- 第三轮的意义是把问题从“评测脏数据”进一步收束到了“模型最终答案生成策略”。

### 下一步

1. 抽样检查训练数据目标答案是否本身过长、过散或隐含推理痕迹。
2. 研究是否需要更强的输出模板约束。
3. 视情况增加生成后处理：
   - 去残句
   - 去空答案
   - 增加最小结构校验
4. 第四轮评测不再以“脚本是否生效”为重点，而要以“答案通道是否变得稳定”为重点。
